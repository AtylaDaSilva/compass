...
export default function Sidebar({ activeTab, onAddTransactionClick }: SidebarProps) {
  const menuItems = [
  ...
  return (
    <aside 
      className={`sidebar-container glass-panel h-100 d-flex flex-column justify-content-between py-4 ${sidebarOpen.state ? "" : "collapsed"}`}
      style={{ 
    ...
    >
      <div>
        {/* Profile Info */}
        <div className={`px-3 mb-4 d-flex align-items-center ${sidebarOpen.state ? "gap-3" : "justify-content-center"}`} style={{ height: "60px" }}>
          <div 
            ...
            return (
              <li key={item.id} className="nav-item">
                <button
                  ...
                  className={`nav-link w-100 d-flex align-items-center border-0 text-start py-3 px-3 position-relative ${sidebarOpen.state ? "gap-3 rounded-3" : "justify-content-center rounded-circle"}`}
                  style={{
                  ...
                  title={!sidebarOpen.state ? item.label : undefined}
                >
                  <IconComponent 
                    size={20} 
                    style={{ 
                  ...
                  />
                  {sidebarOpen.state && <span className="fw-medium">{item.label}</span>}
